...
@section('content')
    @isset(auth()->user()->role->permission['permission']['upazila']['add'])
        <div class="row">
            <div class="col-lg-12">
                    ...
                    <div class="card-body">
                        <form action="{{ route('upazila_insert_form') }}" method="POST" class="form-horizontal">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-7">
                                    <input type="text" name="upazilaName" class="form-control" placeholder="Upazila Name" value="{{ old('upazilaName') }}">
                                    @error('upazilaName')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                ...
                <div class="card-body">
                    <table id="alltableinfo" class="table table-bordered table-striped table-hover custom_table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $upazila->district->districtName }}</td>
                                <td>{{ $upazila->upazilaName }}</td>
                                <td>
                                    @isset(auth()->user()->role->permission['permission']['upazila']['edit'])
                                        <a class="btn-info edit-icon" href="{{ route('upazila_edit_form',$upazila->upazilaId) }}"><i class="mdi mdi-table-edit"></i></a>
                                    @endisset

                                    @isset(auth()->user()->role->permission['permission']['upazila']['delete'])
                                        <a class="btn-danger delete-icon" id="softDelete" data-toggle="modal" data-target="#softDelModal" data-id="{{ $upazila->upazilaId }}" href="#"><i class="mdi mdi-delete"></i></a>
                                    @endisset
                                </td>
                            </tr>
                            @endforeach
